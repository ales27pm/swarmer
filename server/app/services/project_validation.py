...
def native_authoring(value: ProjectResult | Mapping[str, Any]) -> bool:
    """Explicit new-worker state permits drafting, never completion or execution."""
    if isinstance(value, Mapping):
        return value.get("native_validation") == "authoring" and value.get("action") != "complete"
    return value.native_validation == "authoring" and value.action != "complete"
