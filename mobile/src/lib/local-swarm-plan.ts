...
export function buildLocalSwarmPlanPrompt(context: LocalSwarmPlanContext): string {
  ...
  const prompt = [
    ...
    "Pour chercher sur Internet, vérifier l’actualité ou comparer des options actuelles, sélectionne research.query s’il est disponible. Son objectif est envoyé TEL QUEL au moteur de recherche : écris seulement les termes de recherche utiles, avec les contraintes de lieu, langue et date. N’y recopie pas tout le but, les consignes de rédaction ou les interdictions. Conserve exactement le sujet et le lieu du but actuel, sans les remplacer par une activité ou un sujet voisin. Son expected_output contient des titres, URL et extraits de sources. Pour rédiger la réponse sourcée, ajoute writing.draft avec le nœud de recherche comme dépendance obligatoire et les consignes de rédaction dans son objectif : le serveur lui transmet les extraits et URL obtenus. Une recherche de liens seule peut se terminer avec research.query. Sans cet outil, conserve la recherche comme exigence non satisfaite ; un texte généré seul ne prouve pas une recherche.",
    "Pour livrer un plan détaillé, une analyse, un résumé, une traduction ou un document sans demande d’implémentation, sélectionne writing.draft s’il est disponible : ce worker produit le texte demandé, avec les dépendances de recherche nécessaires. Ne transforme pas une demande de plan en construction de code, et ne demande pas à l’utilisateur de rédiger lui-même le document demandé. Une synthèse seule ne peut pas produire ce livrable.",
    "Les exigences de qualité des sources vont dans expected_output et dans l’objectif du rédacteur, pour évaluer les domaines et extraits retournés. N’ajoute pas les qualificatifs de source officielle ou fiable aux termes de recherche : cherche directement le sujet et le lieu, puis évalue les sources obtenues.",
    "Un nœud synthesis effectue uniquement une concaténation déterministe des résultats de ses dépendances, sans appel de modèle. Il ne rédige, ne résume, ne traduit et n’analyse aucun contenu, même après research.query. Confie ces livrables textuels à writing.draft.",
    ...
    "Vérifie chaque nœud research.query : objective doit être une requête de moteur de recherche, pas l’objectif complet du but. Les consignes de réponse vont dans writing.draft.",
