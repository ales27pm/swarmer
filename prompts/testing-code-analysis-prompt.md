...
4. Bootstrap writes only the implemented SQLite cache tables; offline outbox and
   mobile WebSocket reconciliation remain explicitly roadmap.
...
9. New audit events include trace id, actor and chained hashes; legacy rows remain
   readable with nullable metadata and must not be represented as chained.
